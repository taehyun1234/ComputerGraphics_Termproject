...
Vector3 get_normalvec(float x1, float x2, float x3, float y1, float y2, float y3, float z1, float z2, float z3)
{
	Vector3 a;
...
struct Texture_d
{
	...
	int width;
	BMP *bmp;
	struct Texture_d(const char *filename);
};

Texture_d::Texture_d(const char *filename)
{
	/*bmp = new BMP();
...
void OBJModel::Draw(int mode)
{
	...
	for (std::list<Triangle>::const_iterator it = triangleList.begin(); it != triangleList.end(); it++)
	{
		...
		static int k = 0;
		float a, b, c, d, e, f, g, h, l;
		a = vertexArray[(*it).Vertex[k] - 1].x; b = vertexArray[(*it).Vertex[k] - 1].y; c = vertexArray[(*it).Vertex[k] - 1].z;
		d = vertexArray[(*it).Vertex[k + 1] - 1].x; e = vertexArray[(*it).Vertex[k + 1] - 1].y; f = vertexArray[(*it).Vertex[k + 1] - 1].z;
		g = vertexArray[(*it).Vertex[k + 2] - 1].x; h = vertexArray[(*it).Vertex[k + 2] - 1].y; l = vertexArray[(*it).Vertex[k + 2] - 1].z;
		glNormal3d(get_normalvec(a, b, c, d, e, f, g, h, l).x, get_normalvec(a, b, c, d, e, f, g, h, l).y, get_normalvec(a, b, c, d, e, f, g, h, l).z);
